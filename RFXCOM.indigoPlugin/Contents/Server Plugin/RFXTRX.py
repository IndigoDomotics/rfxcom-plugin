...
class RFXTRX(object):
    ...
    def _finalizeStatesChanges(self):
        ...
        for devId in self.batchStatesUpdate:
            ...
            self.plugin.debugLog("finalizeStatesChanges:" + dev.name + " :" + str(self.batchStatesUpdate[devId]))
            try:
                dev.updateStatesOnServer(self.batchStatesUpdate[devId])
            except Exception as e:
                indigo.server.log("_finalizeStatesChanges in Line '%s' has error='%s'" % (sys.exc_traceback.tb_lineno, e) + ", dev: " + dev.name + "  " + str(self.batchStatesUpdate[devId]))

            self.deviceStart(dev, force=True)
        self.batchStatesUpdate = {}
        return
    
    def ord_replace(self, bytes):
        ## Replace all the ord calls with this -- not needed in python3 and 253 of them!
        return bytes
    ...
    def getConfiguration(self, valuesDict):
        ...
        try:
            self.plugin.digitsTemperature = int(valuesDict.get(u'digitsTemperature', 1))
        except:
            self.plugin.digitsTemperature = 1
        try:
            self.plugin.digitsRain = int(valuesDict.get(u'digitsRain', 1))
        except:
            self.plugin.digitsRain = 1
        try:
            self.plugin.digitsWind = int(valuesDict.get(u'digitsWind', 1))
        except:
            self.plugin.digitsWind = 1
    ...
    def SendResetAndInit(self):
        ...
        if replyData[1]!= 0x01 or replyData[2] != 0x00:
        ...
        rfxFirmwareVers = replyData[6]
        rfxEnableUndecoded = replyData[7] & 0x80

        currentProtocolList = []
        enabledProtocolList = []
        disabledProtocolList = []

        curProtoFlags = [replyData[7], replyData[8], replyData[9]]
        ...
        if newProtoFlags != curProtoFlags:
            ...
            while not rcvdSetModeReply:
                ...
                if len(replyData) >= 13 and replyData[1] == 0x01 and replyData[2] == 0x00 and replyData[4] == 0x03:
        ...
        try:
            if self.configRead == True:
                ...
                if (dev.deviceTypeId == "ACAppliance") or (dev.deviceTypeId == "ACDimmer") or (dev.deviceTypeId == "HEAppliance") or (dev.deviceTypeId == "HEDimmer"):
                    ...
                    self.plugin.debugLog(u"Address1 and 2 and 3 = %s , %s , %s " % (adres1, adres2, adres3))
                    # self.plugin.debugLog(u"Ignore Dimmer:"+str(ignoreDimmer))
                    housecode = chr(int(dev.pluginProps['unit']))
                    self.plugin.debugLog("device type:%s" % dev.deviceTypeId)
    ...
    def processPacket(self, data):
        ...
        if len(data) > 1:
            ...
            elif self.ord_replace(data[1]) == 82:
                self.showTemp(data)
            elif self.ord_replace(data[1]) == 84:
                self.showBaro(data)
            elif self.ord_replace(data[1]) == 85:
                self.showRain(data)
            elif self.ord_replace(data[1]) == 16:
                self.triggerSwitch(data)
            elif self.ord_replace(data[1]) == 17:
                self.triggerACSwitch(data)
            elif self.ord_replace(data[1]) == 20:
                self.triggerLWSwitch(data)
            elif self.ord_replace(data[1]) == 24:
                self.triggerRollerTrolRemote(data)
            elif self.ord_replace(data[1]) == 25:
                # self.triggerRollerTrolRemote(data)
                self.triggerBlindsRemote(data)
            elif self.ord_replace(data[1]) == 32:
                self.handleSecurity(data)
            elif self.ord_replace(data[1]) == 48:
                self.triggerPCRemote(data)
            elif self.ord_replace(data[1]) == 87:
                self.handleUV(data)
            elif self.ord_replace(data[1]) == 89:
                self.handleCurrent(data)
            elif self.ord_replace(data[1]) == 90:
                self.handleEnergyUsage(data)
            elif self.ord_replace(data[1]) == 112:
                self.handleRFXSensor(data)
            elif self.ord_replace(data[1]) == 113:
                self.handleRFXMeter(data)
            elif self.ord_replace(data[1]) == 86:
                self.handleWind(data)
            elif self.ord_replace(data[1]) == 2:
    ...
    def handleUndecoded(self, data):
        ...
        # subtype=self.ord_replace(data[2])
        hexres = ''.join(["%02X " % self.ord_replace(char) for char in data]).strip()
        self.plugin.errorLog(u"undecoded packet received: %s" % (hexres))

    def handleChecksum(self, data):
        subtype = self.ord_replace(data[2])
        chktype = self.ord_replace(data[4])
    ...
    def handleSecurity(self, data):
        ...
        subtype = self.ord_replace(data[2])
        device = ""
        status = self.ord_replace(data[7])
        stattext = ""
        if subtype < 3:
            sensor = (self.ord_replace(data[6]) * 256) + self.ord_replace(data[4])
        elif subtype == 3:
            sensor = (self.ord_replace(data[5]) * 256) + self.ord_replace(data[4])
        else:
            sensor = (self.ord_replace(data[6]) * 256 * 256) + (self.ord_replace(data[5]) * 256) + self.ord_replace(data[4])
    ...
    def triggerACSwitch(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        s1 = (hex(self.ord_replace(data[4])) + '0')[2:4]
        s2 = (hex(self.ord_replace(data[5])) + '0')[2:4]
        s3 = (hex(self.ord_replace(data[6])) + '0')[2:4]
        s4 = (hex(self.ord_replace(data[7])) + '0')[2:4]
        s5 = str(100 + self.ord_replace(data[8]))[1:3]
        commcode = self.ord_replace(data[9])
    ...
    def triggerLWSwitch(self, data):
        ...
        s2 = (hex(self.ord_replace(data[5])) + '0')[2:4]
        s3 = (hex(self.ord_replace(data[6])) + '0')[2:4]
        s5 = str(100 + self.ord_replace(data[7]))[1:3]
        commcode = self.ord_replace(data[8])
        level = self.ord_replace(data[9])
    ...
    def triggerLWRemote(self, data):
        ...
        s2 = (hex(self.ord_replace(data[5])) + '0')[2:4]
        s3 = (hex(self.ord_replace(data[6])) + '0')[2:4]
        unit = str(100 + self.ord_replace(data[7]))[1:3]
        commcode = self.ord_replace(data[8])
        level = self.ord_replace(data[9])
    ...
    def triggerPCRemote(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        sensor = (hex(self.ord_replace(data[4])) + '0')[2:4]
        commcode = self.ord_replace(data[5])
    ...
    def triggerSwitch(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        housecode = chr(self.ord_replace(data[4]))
        adres = self.ord_replace(data[5])
        sensor = (self.ord_replace(data[4]) * 100) + self.ord_replace(data[5])
        commcode = self.ord_replace(data[6])
    ...
    def triggerBlindsRemote(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        housecode = (self.ord_replace(data[4]) * 10000) + (self.ord_replace(data[5]) * 100) + self.ord_replace(data[6])
        adres = self.ord_replace(data[7])
        sensor = int((self.ord_replace(data[4]) * 1000000) + (self.ord_replace(data[5]) * 10000) + (self.ord_replace(data[6]) * 100) + self.ord_replace(data[7]))
        ...
        hexhouse = ''.join(["%02X" % self.ord_replace(char) for char in data[4:7]]).strip()
        subtype = int(subtype)
        self.plugin.debugLog(u"Enter the below into Device Details:  HouseCode (hex):" + str(hexhouse) + u', Subtype (int):' + str(subtype) + u'& UnitCode (int):' + str(int(adres)))
        if commcode == 0:
            commando = "Open"
        elif commcode == 1:
            commando = "Close"
    ...
    def showRain(self, data):
        ...
        rainrate = (self.ord_replace(data[6]) << 8) + self.ord_replace(data[7])
        # RAIN1 rate is mm/hr
        ...
        raintotal = (self.ord_replace(data[8]) << 16) + (self.ord_replace(data[9]) << 8) + self.ord_replace(data[10])
        rainrate = self.convertRainfallToUnits(rainrate)
        raintotal = self.convertRainfallToUnits(raintotal / 10)
        sensor = (self.ord_replace(data[5]) << 8) + self.ord_replace(data[4])
    ...
    def handleRFXSensor(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        sensor = self.ord_replace(data[4])
        svalue = (self.ord_replace(data[5]) * 256) + self.ord_replace(data[6])
        humid = 0
        temp = 0

        self.plugin.debugLog(u"RFXSensor %d with subtype %d and value %d" % (sensor, subtype, svalue))
        self.plugin.debugLog(u"Data1: %s %s %s %s %s %s %s %s" % (self.ord_replace(data[0]), self.ord_replace(data[1]), self.ord_replace(data[2]), self.ord_replace(data[3]), self.ord_replace(data[4]), self.ord_replace(data[5]), self.ord_replace(data[6]), self.ord_replace(data[7])))
        if sensor in self.devicesCopy.keys():
            self.plugin.debugLog(u"RFXSensor %d in list" % sensor)
            dev = self.devicesCopy[sensor]
            if subtype == 0:
                temp = self.ord_replace(data[6])
                vFactor = 1
                temp2 = self.ord_replace(data[5])
                if self.ord_replace(data[5]) > 127:
                    temp2 = self.ord_replace(data[5]) - 128
                    vFactor = -1
                temp += (temp2 * 256)
    ...
    def handleRFXMeter(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        sensor = (self.ord_replace(data[4]) * 256) + self.ord_replace(data[5])
        self.plugin.debugLog(u"RFXMeter %d with subtype %d" % (sensor, subtype))
        self.plugin.debugLog(u"Data: %s %s %s %s" % (self.ord_replace(data[6]), self.ord_replace(data[7]), self.ord_replace(data[8]), self.ord_replace(data[9])))
        ...
        if sensor in self.devicesCopy.keys():
            ...
            if subtype == 0:
                thisValue = (self.ord_replace(data[6]) * 256 * 256 * 256) + (self.ord_replace(data[7]) * 256 * 256) + (self.ord_replace(data[8]) * 256) + self.ord_replace(data[9])
                if self.checkIfNewDay(sensor):
    ...
    def showTemp(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        temp = self.ord_replace(data[7])
        vFactor = 1
        temp2 = self.ord_replace(data[6])
        if self.ord_replace(data[6]) > 127:
            temp2 = self.ord_replace(data[6]) - 128
            vFactor = -1
    ...
    def showBBQ(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        temp1 = self.ord_replace(data[7])
        vFactor = 1
        temp3 = self.ord_replace(data[6])
        if self.ord_replace(data[6]) > 127:
            temp3 = self.ord_replace(data[6]) - 128
            vFactor = -1
        ...
        temp2 = self.ord_replace(data[9])
        vFactor = 1
        temp3 = self.ord_replace(data[8])
        if self.ord_replace(data[8]) > 127:
            temp3 = self.ord_replace(data[8]) - 128
            vFactor = -1
    ...
    def showBaro(self, data):
        ...
        baro = (self.ord_replace(data[10]) * 256) + self.ord_replace(data[11])
    ...
    def handleWind(self, data):
        ...
        direction = (self.ord_replace(data[6]) << 8) + self.ord_replace(data[7])
        avgSpeed = float(((self.ord_replace(data[8]) << 8) + self.ord_replace(data[9]))) / 10
        gust = float(((self.ord_replace(data[10]) << 8) + self.ord_replace(data[11]))) / 10
        ...
        if (subtype == 4):
            temp = self.ord_replace(data[13])
            vFactor = 1
            temp2 = self.ord_replace(data[12])
            if self.ord_replace(data[12]) > 127:
                temp2 = self.ord_replace(data[12]) - 128
                vFactor = -1

            temp += (temp2 * 256)
            temp = temp * vFactor * 0.1

            chill = self.ord_replace(data[15])
            vFactor = 1
            chill2 = self.ord_replace(data[14])
            if self.ord_replace(data[14]) > 127:
                chill2 = self.ord_replace(data[14]) - 128
                vFactor = -1
    ...
    def showHumidity(self, data):
        sensorType = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        ...
        batteryAndSignalData = data[8]
        humid = self.ord_replace(data[6])

        batteryLevel = self.getBatteryLevel(batteryAndSignalData)
        signalStrength = self.getSignalStrength(batteryAndSignalData)

        sensor = (self.ord_replace(data[5]) * 256) + self.ord_replace(data[4]) + self.ord_replace(data[1])
    ...
    def handleUV(self, data):
        ...
        temp = 0
        if subtype == 3:
            temp = self.ord_replace(data[8])
            vFactor = 1
            temp2 = self.ord_replace(data[7])
            if self.ord_replace(data[7]) > 127:
                temp2 = self.ord_replace(data[7]) - 128
                vFactor = -1
            temp += (temp2 * 256)
            temp = temp * vFactor * 0.1

        UVLevel = float(self.ord_replace(data[6])) / 10
        ...
        if sensor in self.devicesCopy.keys():
            ...
            if displayMode == "UvTemp":
                ...
                display = "%.1f (%s) / %s (%s - %s)" % (
                UVLevel, float(self._getCurrentSensorValue(self.devicesCopy[sensor], "maxUVLevel")), self.temperatureToString(temp), self.temperatureToString(self._getCurrentSensorValue(self.devicesCopy[sensor], "mintemperature")), self.temperatureToString(self._getCurrentSensorValue(self.devicesCopy[sensor], "maxtemperature")))
            self._addToBatchStatesChange(self.devicesCopy[sensor], key=u"display", value=display)
        # self._finalizeStatesChanges()
        else:
            self.handleUnknownDevice(devicetype, sensor)
    ...
    def handleCurrent(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        Current1 = float((self.ord_replace(data[7]) * 256) + self.ord_replace(data[8])) / 10
        Current2 = float((self.ord_replace(data[9]) * 256) + self.ord_replace(data[10])) / 10
        Current3 = float((self.ord_replace(data[11]) * 256) + self.ord_replace(data[12])) / 10
    ...
    def handleEnergyUsage(self, data):
        devicetype = self.ord_replace(data[1])
        subtype = self.ord_replace(data[2])
        CurrentUsage = float((self.ord_replace(data[7]) * 256 * 256 * 256) + (self.ord_replace(data[8]) * 256 * 256) + (self.ord_replace(data[9]) * 256) + self.ord_replace(data[10]))
        TotalkWh = float((self.ord_replace(data[11]) * 256 * 256 * 256 * 256 * 256) + (self.ord_replace(data[12]) * 256 * 256 * 256 * 256) + (self.ord_replace(data[13]) * 256 * 256 * 256) + (self.ord_replace(data[14]) * 256 * 256) + (self.ord_replace(data[15]) * 256) + self.ord_replace(data[16])) / 223.666
    ...
    def logdata(self, packdata, debugstr):
        ...
        #res = ''.join(["%d " % self.ord_replace(char) for char in packdata]).strip()
       # hexres = ''.join(["%02X " % self.ord_replace(char) for char in packdata]).strip()


        self.plugin.debugLog(u"{} ({})".format(packdata, debugstr))
    ...
    def checkIfNewDay(self, sensor):
        try:
            ...
            if (lastDateTime != ""):
                ...
                # indigo.server.log(u"check if new day: last date %s, new date %s" % (lastDateTime[0:compTime], newdates[0:compTime])+" !=: "+str(lastDateTime[0:compTime]!=newdates[0:compTime]))
    ...
    def getBatteryLevel(self, data):
        return (self.ord_replace(data) & 15) + 1

    def getToggleLevel(self, data):
        return (self.ord_replace(data) & 1)

    def getcmdTypeLevel(self, data):
        return ((self.ord_replace(data) & 14) >> 1)

    def getSignalStrength(self, data):
        return (self.ord_replace(data) >> 4)

    def getHumidityStatus(self, data):
        if self.ord_replace(data) == 0:
            return "dry"
        elif self.ord_replace(data) == 1:
            return "comfort"
        elif self.ord_replace(data) == 2:
            return "normal"
        elif self.ord_replace(data) == 3:
            return "wet"
